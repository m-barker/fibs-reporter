...
class DataAnalyser:
    ...
    def get_output_correlations(self, audio_file: str = None, ) -> tuple:
        ...
        # Adjust p-values to account for FDR (false discovery rate)
        _, spearman["P-value"] = fdrcorrection(spearman["P-value"], is_sorted=True)
        _, kruskal["P-value"] = fdrcorrection(kruskal["P-value"], is_sorted=True)
